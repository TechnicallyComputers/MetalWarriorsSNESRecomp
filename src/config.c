...
static bool HandleIniConfig(int section, const char *key, char *value) {
  if (section == 0) {
    ...
    } else if (StringEqualsNoCase(key, "InputDevice1") ||
               StringEqualsNoCase(key, "InputDevice2")) {
      int p = StringEqualsNoCase(key, "InputDevice2") ? 1 : 0;
      snprintf(g_config.input_device[p], sizeof(g_config.input_device[p]), "%s", value);
      /* Keep EnableGamepad in sync so OpenOneGamepad works without the launcher. */
      if (g_config.input_device[p][0] &&
          !StringEqualsNoCase(g_config.input_device[p], "none") &&
          !StringEqualsNoCase(g_config.input_device[p], "keyboard"))
        g_config.enable_gamepad[p] = true;
      else if (g_config.input_device[p][0])
...
void ParseConfigFile(const char *filename) {
  ...
  g_config.input_device[0][0] = '\0';
  g_config.input_device[1][0] = '\0';
...
void WriteConfigFile(const char *filename) {
  ...
  CfgKV kvs[] = {
    ...
    { "GamepadMap", "InputDevice1" },
    { "GamepadMap", "InputDevice2" },
  ...
  snprintf(kvs[3].val, sizeof(kvs[3].val), "%d", (int)g_config.fullscreen);
  ...
  snprintf(kvs[5].val, sizeof(kvs[5].val), "%d", g_config.ignore_aspect_ratio ? 1 : 0);
  snprintf(kvs[6].val, sizeof(kvs[6].val), "%d", g_config.enable_audio ? 1 : 0);
  snprintf(kvs[7].val, sizeof(kvs[7].val), "%d", g_config.audio_freq);
  snprintf(kvs[8].val, sizeof(kvs[8].val), "%d", g_config.msu1_enabled ? 1 : 0);
  snprintf(kvs[9].val, sizeof(kvs[9].val), "%s", g_config.msu1_dir);
  snprintf(kvs[10].val, sizeof(kvs[10].val), "%s", g_config.enable_gamepad[0] ? "true" : "false");
  snprintf(kvs[11].val, sizeof(kvs[11].val), "%s", g_config.enable_gamepad[1] ? "true" : "false");
  snprintf(kvs[12].val, sizeof(kvs[12].val), "%s", g_config.input_device[0]);
  snprintf(kvs[13].val, sizeof(kvs[13].val), "%s", g_config.input_device[1]);
  snprintf(kvs[14].val, sizeof(kvs[14].val), "%d", g_config.skip_launcher ? 1 : 0);
  snprintf(kvs[15].val, sizeof(kvs[15].val), "%s", g_config.netplay_player_name);
  snprintf(kvs[16].val, sizeof(kvs[16].val), "%d", g_config.gamepad_deadzone);
